...
| T4 | A public repo reveals `sapserver`'s address/identity or makes it reachable | Opening the repo | Opening the repo does NOT open `sapserver` (`specs/003` acceptance: "Installing the open-source plugin grants no access to sapserver"; `specs/002`). Before publishing: grep the repo for `100.112.246.26`, `sapserver`, Tailscale names, private vault paths. Sapserver stays behind Tailscale, without Funnel (Funnel = a gate requiring a question, `plan/01` rule 9). |
