...
describe('failure reporting never logs provider payloads', () => {
  it('logs only a stable diagnostic message', () => {
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    try {
      pane({ ...HEALTHY, panelProvider: boom });
      expect(errorSpy).toHaveBeenCalledWith(
        'Havemind: provider "panel" failed.',
      );
      expect(errorSpy.mock.calls.every((call) => call.length > 0)).toBe(true);
      expect(errorSpy.mock.calls.every((call) => call.length === 1)).toBe(true);
    } finally {
