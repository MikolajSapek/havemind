...
describe('fetchMemberRoster', () => {
  ...
  it('prefers GET /vaults/:id/members with a Bearer access token when one is available', async () => {
    const { calls, fn } = fakeRequestUrl({
      status: 200,
      json: {
        members: [
    ...
    ).toBe('Bearer hm_at_live');
    expect(members.map((member) => member.membershipId).sort()).toEqual([
      'm-owner',
      'm-phone',
    ]);
    expect(members.find((member) => member.self)?.displayName).toBe('You');
  });

  it('falls back to GET /members when the Bearer roster omits membership ids', async () => {
    const calls: string[] = [];
    const fn: RequestUrlFn = async (options) => {
      calls.push(options.url);
      if (options.url.includes('/vaults/')) {
        return {
          status: 200,
          json: {
            members: [{ displayName: 'Mikolaj', role: 'owner' }],
            version: 1,
          },
        };
      }
      return {
        ...
        json: {
          members: [
            { membershipId: 'm-owner', displayName: 'Mikolaj', role: 'owner' },
            { membershipId: 'm-phone', displayName: 'Telefon', role: 'editor' },
          ],
          version: 1,
        },
      };
    ...
    expect(calls).toEqual([
      `${API}/vaults/${VAULT}/members`,
      `${API}/members?vault=${VAULT}`,
    ]);
