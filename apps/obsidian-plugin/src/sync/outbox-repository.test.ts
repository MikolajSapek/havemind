...
describe('binary mappings carry no base64 body (AUD-12)', () => {
  // A mapping exists to answer "did this path change?", which the hash alone
  // settles. Binary files never merge, so their bytes in the mapping are dead
  // weight, and base64 is a third larger than the file itself. Four PDFs and a
  // dozen images grew one real vault's data.json to 9.4 MB, of which 8.15 MB
  // was binary base64. Obsidian rewrites that file whole on every save; when
  // such a write is interrupted the state parses as corrupt, the cursor resets
  // to 0, and the client replays the whole history, resurrecting deleted notes.
  it('stores the hash but not the bytes for a binary file', async () => {
    const { repo, store } = makeRepo();
    await repo.commitLocalChange({
      operation: makeOperation({
        content: 'QUJD',
    ...
    const mapping = (await store.load()).mappings.find((m) => m.fileId === FILE_ID);
    expect(mapping?.contentHash).toBe('hash-bin');
    expect(mapping?.content).toBeNull();
  });

  it('still stores the text of a markdown file, which merges', async () => {
    const { repo, store } = makeRepo();
    await repo.commitLocalChange({
      operation: makeOperation(),
      removeFileId: null,
    ...
    const mapping = (await store.load()).mappings.find((m) => m.fileId === FILE_ID);
    expect(mapping?.content).toBe('Hello\n');
  });
});
